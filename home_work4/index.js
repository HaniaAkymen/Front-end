...
if (order_cost2 >= 100) {
    ...
    alert(`К сожалению вашего заказа не достаточно дл получения скидки. Сумма к оплате - ${(order_cost2*0.95).toFixed(2)}`);
};
}, 500);
...
if (user_number > 0) {
    alert(`Поздравляем. Сумма вашего заказа - ${(user_number).toFixed(2)} больше 0`);
} else if (user_number < 0){
    alert(`Поздравляем. Сумма вашего заказа - ${(user_number).toFixed(2)} меньше 0`);
} else {
